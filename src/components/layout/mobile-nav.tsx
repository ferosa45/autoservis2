...
import { CalendarDays, ClipboardList, Users, FileText } from 'lucide-react';
...
export async function MobileNav() {
  ...
  const visibleItems = NAV_ITEMS.filter(
    (item) => !item.permission || context.role === 'OWNER' || context.permissions[item.permission]
  );
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 px-1 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden">
      <div className="flex h-16 items-stretch overflow-x-auto">
        <div className="flex min-w-full items-stretch justify-around gap-1">
          {visibleItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="flex min-w-[68px] flex-1 flex-col items-center justify-center gap-1 px-2 text-[11px] font-medium text-text-secondary active:bg-elevated"
              >
          ...
          <div className="flex min-w-[76px] flex-1 items-center px-1">
            <NewJobButton />
          </div>
        </div>
      </div>
