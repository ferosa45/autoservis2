...
function daysLeft(date: Date) { return Math.max(0, Math.ceil((date.getTime() - Date.now()) / DAY_MS)); }
...
export default async function AdminGarageDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const garage = await prisma.garage.findUnique({ where: { id }, include: { users: { orderBy: [{ role: 'asc' }, { createdAt: 'asc' }] }, _count: { select: { users: true, customers: true, vehicles: true, jobs: true, invoices: true } } });
  if (!garage) notFound();
  const owner = garage.users.find((user) => user.role === 'OWNER');
  const trialDays = daysLeft(garage.trialEndsAt);
  const resetTrial = resetGarageTrial.bind(null, garage.id);
